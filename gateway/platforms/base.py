...
def get_image_cache_dir() -> Path:
    ...
    cache_dir = IMAGE_CACHE_DIR or (get_hermes_home() / "image_cache")
...
def cache_image_from_path(source_path: str | Path, ext: str | None = None) -> str:
    """Copy an image file into the cache and return the cached absolute path."""
...
def get_audio_cache_dir() -> Path:
    ...
    cache_dir = AUDIO_CACHE_DIR or (get_hermes_home() / "audio_cache")
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir
...
def get_document_cache_dir() -> Path:
    ...
    cache_dir = DOCUMENT_CACHE_DIR or (get_hermes_home() / "document_cache")
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir
...
def cache_document_from_path(source_path: str | Path, filename: str | None = None) -> str:
    """Copy a document file into the cache and return the cached absolute path."""
    source = Path(source_path)
    cache_dir = get_document_cache_dir()
...
def build_text_attachment_injection(raw_bytes: bytes, display_name: str, ext: str) -> Optional[str]:
    """Return injected prompt text for supported text attachments.

    Small UTF-8 text files are injected in full. Large CSV/TSV files are
    previewed so the agent still sees the schema and first rows without
    flooding context.
    """
    ext = (ext or "").lower()
    ...
    return (
        f"[Preview of {safe_name}]:\n{preview}\n\n"
        "[The preview is truncated because the original attachment is large.]"
    )


def build_text_attachment_injection_from_path(
    path: str | Path,
    *,
    display_name: str | None = None,
) -> Optional[str]:
    """Return injected prompt text for a supported text attachment on disk."""
    file_path = Path(path)
