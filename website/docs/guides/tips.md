...
Press **Ctrl+C** once to interrupt the agent mid-response. If you want to keep the current task running, type a new message and press Enter to queue it for later; press `Esc` or type `/stop` when you need to preempt immediately. Double-press Ctrl+C within 2 seconds to force exit.
