...
If the agent is taking too long, type a new message and press Enter to queue it for the next turn. Press `Esc` or type `/stop` to preempt the current task, or `Ctrl+C` to stop the current operation.
